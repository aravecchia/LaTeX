...
\usepackage[paperwidth=1189mm, paperheight=841mm, margin=0mm]{geometry} % Define o tamanho da folha (A0) e sem margens
...
\begin{document}
	\foreach \i in {1,...,10} { % Itera de 1 até 10 para gerar múltiplas páginas
		\begin{tikzpicture}[remember picture, overlay, x=1mm, y=1mm]
			...
			\foreach \dx in {210, 420, 630, 840, 1050} {
				\draw[line width=1mm]
...
Edite o selo como desejar, no seu editor \LaTeX\space preferido. Gere o pdf, converta-o para SGV (use \textit{pdf2svg} se você está em Linux) e exporte-o para a pasta de folhas da bancada Techdraw, no FreeCAD, e divirta-se!
					...
					\node at (105,110) {\includegraphics[width=15mm]{./IMG/tux.png}}; % Logotipo Linux
						...
						% === LICENÇA ===
%		\node[anchor=west] at (10,50) {
%						\begin{minipage}[t]{110mm} % Caixa de texto
%							\small
%	 CC \(CreativeCommons \)
%%é o mais óbvio, pois indica que a licença usada é a Creative Commons;
%	% BY \(Atribuição \)
%%significa que você deve dar os devidos créditos ao autor ou responsável pela obra, devendo inclusive fornecer um link para a licença e indicar se realizou alterações no material;
%	NC \(Não Comercial \)
%%você pode usar o material, mas não para fins comerciais (será necessário negociar com o autor ou detentor para isso);
%	ND \(Sem Derivações \)
%%	você até pode criar obras derivadas a partir do material original, mas não distribuí-las como tal;
%	% SA \(Compartilhar Igual )
%	%se você criar obras derivadas, terá que distribuí-las sob a mesma licença usada na original.
%\end{minipage}			
					\node at (162.5,50)
%{\includegraphics[width=40mm]{./IMG/BY-CC-SA.png}};
%{\includegraphics[width=40mm]{./IMG/BY-NC-ND.png}};
{\includegraphics[width=40mm]{./IMG/BY-NC.png}};
%{\includegraphics[width=40mm]{./IMG/BY-ND.png}}; {\includegraphics[width=40mm]{./IMG/BY-SA.png}};				
